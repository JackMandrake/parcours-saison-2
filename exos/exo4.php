...
function displayStatusHTTP($httpStatusList) {
    ...
    foreach ($httpStatusList as $code => $message) {
      $html .= '<li>';
...
/**
 * 1er point que je n'ai pas compris : la variable $httpStatusList permettais de regrouper les 2 listes ci dessous
 * donc pas besoin de copier les listes pour qu'elles apparaissent avant
 * 
 * 2ème point que je n'ai pas compris : en suivant l'auto correction, mon code était bon jusqu'à l'attribution de
 * la variable $html. J'ai fais ceci :
 *  $html = '<li>'.'<strong>'.$code.'</strong>'.$message.'</li>';
 * j'aurais du utilisé l'opérateur d'affectation qui aurais permis d'ajouté les éléments bout à bout de ma variable 
 * $html
 * Je devrais poser la question pour avoir de plus amples explications à ce sujet
 */
